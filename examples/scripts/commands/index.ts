...
program
  .name('ERC-4337 Lyfebloc Network Implementation')
  .description('A collection of example scripts for working with ERC-4337 Lyfebloc Network Implementation')
  .version('0.1.0');
...
program
  ...
  .action(async (opts) => erc20Transfer(opts.token, opts.to, opts.amount));
...
program
  ...
  .requiredOption('-amt, --amount <decimal>', 'Amount of the token to transfer')
  .action(async (opts) => erc20Approve(opts.token, opts.spender, opts.amount));
...
program
  ...
  .requiredOption('-amt, --amount <decimal>', 'Amount of the token to transfer')
  .action(async (opts) => batchErc20Transfer(opts.token, opts.to.split(','), opts.amount));
...
program
  ...
  .action(async (opts) => nftTransfer(opts.token, opts.to, opts.tokenId));
